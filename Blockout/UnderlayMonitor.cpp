...
struct Target
{
    Target(HWND overlayWindow)
        : pid(0), overlayWindow(overlayWindow), underlayWindow(0), underlayHandle(INVALID_HANDLE_VALUE),
          windowArea(0){};
    ...
    void UpdateOverlay()
    {
        ...
        SetWindowPos(overlayWindow, // hWnd
                     0,             // hWndInsertAfter
                     rect.left,     // X
                     top,           // Y
                     width,         // cx
        ...
        SetWindowPos(overlayWindow,                           // hWnd
                     ...
                     0,                                       // cx
                     0,                                       // cy
                     SWP_SHOWWINDOW | SWP_NOMOVE | SWP_NOSIZE // uFlags
        );
    }
 ...
 * @param lparam Target* containing the pid to locate. Sets underlay field when
 * found. If the process has multiple windows, the larget window is returned.
...
struct UnderlayMonitor::Impl
{
     ...
     */
    static void CALLBACK UnderlayChanged(HWINEVENTHOOK hook, DWORD event, HWND hwnd, LONG idObject, LONG idChild,
                                         DWORD dwEventThread, DWORD dwmsEventTime);
};

UnderlayMonitor::Impl::Impl(HWND overlay) : currentProcessName(L"")
{
...
bool UnderlayMonitor::Impl::Connect(const std::wstring processName)
{
    ...
    hooks.push_back(SetWinEventHook(EVENT_OBJECT_LOCATIONCHANGE, // eventMin
    ...
    auto success = std::all_of(hooks.begin(), hooks.end(), [](HWINEVENTHOOK hook) { return hook != nullptr; });
...
void UnderlayMonitor::Impl::UnderlayChanged(HWINEVENTHOOK hook, DWORD event, HWND hwnd, LONG idObject, LONG idChild,
                                            DWORD dwEventThread, DWORD dwmsEventTime)
{
